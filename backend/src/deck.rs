...
fn delete_deck(id: i32) -> Result<status::NoContent, status::Custom<String>> {
    ...
    db_connection.transaction::<_, diesel::result::Error, _>(|conn| {
        // First, delete all cards associated with this deck
        let num_cards = diesel::delete(card::table.filter(card::deck_id.eq(id)))
            .execute(conn)?;

        rocket::info!("Deleted {} cards from deck {}", num_cards, id);

        // Then delete the deck itself
        let num_decks = diesel::delete(deck::table.filter(deck::id.eq(id)))
            .execute(conn)?;
    ...
    .map_err(|e| match e {
        diesel::result::Error::NotFound => status::Custom(
            Status::NotFound,
            format!("Deck with id {} not found", id)
        ),
        _ => {
